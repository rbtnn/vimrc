...
function! lsfiles#popupwin#apply_border(opts, hlname) abort
    if has('gui_running') || (!has('win32') && !has('gui_running'))
        if hlexists(a:hlname)
            call extend(a:opts, {
                ...
                \ 'borderchars': get(g:, 'lsfiles_popupwin_border_type', 1) ? s:borderchars_typeA : s:borderchars_typeB,
                \ }, 'force')
        endif
    endif
    return a:opts
...
function! lsfiles#popupwin#apply_size(opts) abort
    let width = 120
    let height = 20 + 4
...
function! lsfiles#popupwin#set_cursorline(winid, lnum) abort
