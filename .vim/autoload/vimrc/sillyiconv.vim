...
function vimrc#sillyiconv#utf_8(input)
...
function vimrc#sillyiconv#euc_jp(input)
...
function vimrc#sillyiconv#shift_jis(input)
...
function vimrc#sillyiconv#iso_2022_jp(input)
...
function vimrc#sillyiconv#of(input)
	if vimrc#sillyiconv#iso_2022_jp(a:input)
		return "iso-2022-jp"
	elseif vimrc#sillyiconv#utf_8(a:input)
		return "utf-8"
	elseif vimrc#sillyiconv#euc_jp(a:input)
		return "euc-jp"
	elseif vimrc#sillyiconv#shift_jis(a:input)
		return "shift_jis"
	else
...
function vimrc#sillyiconv#iconv_one_nothrow(val)
	let val = a:val
	try
		let val = vimrc#sillyiconv#iconv_one(val)
	catch
	endtry
	return val
endfunction

function vimrc#sillyiconv#iconv_one(val)
	return get(vimrc#sillyiconv#iconv([(a:val)]), 0, '')
endfunction

function vimrc#sillyiconv#iconv(lines, ...)
	let to_encode = a:0 > 0 ? a:1 : &encoding
	let lines = copy(a:lines)
	for i in range(0, len(lines) - 1)
		if 0 < len(filter(split(lines[i], '\zs'), {i,x -> 0x80 < char2nr(x) }))
			let lines[i] = iconv(lines[i], vimrc#sillyiconv#of(lines[i]), to_encode)
		endif
	endfor
...
function vimrc#sillyiconv#system(cmd)
	return join(vimrc#sillyiconv#iconv(split(system(a:cmd), "\n", 1)), "\n")
endfunction

function vimrc#sillyiconv#qficonv()
	let xs = getqflist()
	for x in xs
		if has_key(x, 'text')
			let x['text'] = vimrc#sillyiconv#iconv_one_nothrow(x['text'])
		endif
	endfor
	call setqflist(xs)
endfunction
